...
if (!empty($groupedData)) {
    ...
    foreach ($groupedData as $groupName => $groupInfo) {
        ...
        if (!$isSinGrupo) {
            ...
            $card_body .= ' <span class="badge badge-success ml-2 font-weight-bold font-24">$' . number_format($groupInfo['total'], 0, ',', '.') . '</span>';
            $card_body .= '</button>';
        } else {
            $card_body .= '<button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#' . $collapseId . '">';
            ...
            $card_body .= ' <span class="badge badge-warning ml-2 font-weight-bold font-24">$' . number_format($groupInfo['total'], 0, ',', '.') . '</span>';
            $card_body .= '</button>';
        }
        $card_body .= '</h5>';
        $card_body .= '</div>';
